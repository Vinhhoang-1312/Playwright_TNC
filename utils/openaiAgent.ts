...
import { autoFixLocator, setLocatorFixer } from './locatorAutoFixer';
...
New selector:`;
  ...
  try {
    // Nếu không có DOM, dùng heuristic luôn, không gọi AI
    if (!domHtml) {
      console.log('[OpenAI] No DOM, skipping AI, using heuristic...');
      suggestion = getHeuristicLocator('', oldLocator || '');
      console.log('[OpenAI] Heuristic suggestion:', suggestion);
    } else {
      // Có DOM, gọi AI với timeout 30s
    ...
    console.log('[OpenAI] Falling back to heuristic locator...');
...
function getHeuristicLocator(domHtml: string, oldLocator: string = ''): string | null {
  try {
    console.log('[OpenAI][Fallback] Trying heuristic selector detection...');
    console.log('[OpenAI][Fallback] Old locator:', oldLocator);
    ...
    if (!domHtml && oldLocator) {
      ...
      if (oldLocator.includes('js-login-email')) {
        ...
        console.log('[OpenAI][Fallback] Pattern-based fix:', fixed);
        return fixed;
      }
      ...
      if (withoutSuffix !== oldLocator) {
        console.log('[OpenAI][Fallback] Removed -xyz suffix:', withoutSuffix);
    ...
    if (domHtml) {
      ...
      if (oldLocator.toLowerCase().includes('password')) {
        ...
        if (passwordSelector) {
          console.log('[OpenAI][Fallback] Found password-like selector:', passwordSelector);
      ...
      if (oldLocator.toLowerCase().includes('email') || oldLocator.toLowerCase().includes('mail')) {
        ...
        if (emailSelector) {
          console.log('[OpenAI][Fallback] Found email-like selector:', emailSelector);
      ...
      if (oldLocator.toLowerCase().includes('button') || oldLocator.toLowerCase().includes('submit') || oldLocator.toLowerCase().includes('login')) {
        ...
        if (buttonSelector) {
          console.log('[OpenAI][Fallback] Found button-like selector:', buttonSelector);
